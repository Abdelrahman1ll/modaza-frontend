  ...
  <>
    {Array.from({ length: 6 }).map((_, index) => (
      <div key={index} className="bg-transparent flex flex-col animate-pulse">
        <div className="w-full h-[460px] rounded-[2.5rem] bg-gray-200 mb-4" />
    ...
    <div className="py-14 m-4">
      <section className="max-w-7xl mx-auto px-4">
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="grid grid-cols-1 min-[600px]:grid-cols-2 lg:grid-cols-3 gap-10"
        >
          {isLoading ? (
            <WishlistSkeleton />
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative aspect-[3/4.2] w-full overflow-hidden rounded-[2.5rem] bg-gray-50 transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover/card:rounded-4xl">
                      ...
                      {/* Wishlist Toggle - Premium Glass Circle */}
                      <motion.button
                        onClick={() => handleToggleWishlist(product.id)}
                        whileHover={{ scale: 1.15, rotate: 5 }}
                        whileTap={{ scale: 0.85 }}
                        className="absolute top-6 right-6 z-30 flex h-12 w-12 items-center justify-center rounded-full border border-white/40 bg-white/20 shadow-2xl backdrop-blur-xl transition-all duration-300 hover:bg-white/40 cursor-pointer"
