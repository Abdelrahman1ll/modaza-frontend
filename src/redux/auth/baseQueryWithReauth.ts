    ...
    */
    ...
    if (staticData[path]) {
      ...
      if (path === "/products" && queryString) {
        ...
        let productList = Array.isArray(dataNode) ? dataNode : dataNode.products || [];
        ...
        if (minPrice) {
           productList = productList.filter((p: any) => p.price >= Number(minPrice));
        }
        
        const maxPrice = params.get("maxPrice");
        if (maxPrice) {
           productList = productList.filter((p: any) => p.price <= Number(maxPrice));
        }
    ...
    for (const key in staticData) {
      if (path.startsWith(key + "/")) {
        ...
        if (Array.isArray(dataArray)) {
          ...
          if (key === "/reviews") {
            const productReviews = dataArray.filter((i: any) => String(i.productId) === String(id));
